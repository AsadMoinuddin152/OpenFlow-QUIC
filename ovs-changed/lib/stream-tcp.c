...
int (*INITCLIENT)(const char*);
int (*CLOSE)(int);
static int init_client_lib_finish = 0;
VLOG_DEFINE_THIS_MODULE(stream_tcp);

void *fhandle;


static void init_client_lib();

/* initializing dynamic lib. */
static void 
init_client_lib(){
    if (init_client_lib_finish == 0){
        fhandle =  dlopen("/tmp/libclient.so", RTLD_GLOBAL|RTLD_LAZY );
        INITCLIENT = dlsym(fhandle,"InitClient");
        CLOSE = dlsym(fhandle,"Close");
        init_client_lib_finish = 1 ;
    }
}
...
new_tcp_stream(char *name, int fd, int connect_status, struct stream **streamp)
{
    ...
    if (connect_status == 11 ) {
    	VLOG_INFO("net_tcp_stream:%d",fd);
...
tcp_open(const char *name, char *suffix, struct stream **streamp, uint8_t dscp)
{
    ...
    if (strchr(suffix, ':') != 0)
    {
        error = inet_unix_quic_open(suffix,&fd,0);
    	}
    //if suffix not null    
    else
    {
        char * addr = strcat(suffix,":6653");
        VLOG_INFO("Connection addr:%s",addr);
        error = inet_unix_quic_open(suffix,&fd,0);
    } 
    if (fd >= 0) {
    ...
    if (error == 0) {
        ...
        VLOG_ERR("%s: connect error!!!: %s", name, ovs_strerror(error));
        return error;
    }
}
