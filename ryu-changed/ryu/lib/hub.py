...
quic = cdll.LoadLibrary('/tmp/libserver.so')  
quic.InitQuicServerManager.argtypes = [ctypes.c_char_p]
quic.InitQuicServerManager.restype = ctypes.c_int

quic.CloseClient.argtypes = [ctypes.c_char_p]
quic.CloseClient.restype = ctypes.c_int
...
if HUB_TYPE == 'eventlet':
    ...
    class StreamServer(object):
        def __init__(self, listen_info, handle=None, backlog=None,
            ...
            LOG.info("hub.py StreamServer listen_info:{0}".format(listen_info))
            LOG.info("handler = {0}\n".format(handle))
            if ip.valid_ipv6(listen_info[0]):
                ...
                try:
                    os.unlink(listen_info[0])
                except FileNotFoundError:
                    LOG.info("unix domain socket clean{0}".format(listen_info[0]))
                    
                LOG.info("AF_UNIX")
                unixpath = bytes(listen_info[0], encoding='utf8')
                quic.InitQuicServerManager(unixpath)
