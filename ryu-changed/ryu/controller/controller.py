...
DEFAULT_OFP_GOQUIC = "/tmp/ryu_quic_go.sock"
...
CONF.register_cli_opts([
    ...
    cfg.StrOpt('ofp-quic-unix-listen', default= DEFAULT_OFP_GOQUIC,
               help='openflow quic-go listen %s' %DEFAULT_OFP_GOQUIC),
])
CONF.register_opts([
...
class OpenFlowController(object):
    ...
    # entry point
    def __call__(self):
        if CONF.ofp_quic_unix_listen is not None:
            self.server_loop(ofp_quic_unix_listen=CONF.ofp_quic_unix_listen)
    ...
    def server_loop(self, ofp_tcp_listen_port=None, ofp_ssl_listen_port=None,ofp_quic_unix_listen=None):
        if ofp_quic_unix_listen is not None:
            LOG.info("server_loop\n")
            server = StreamServer((ofp_quic_unix_listen,
                            DEFAULT_OFP_GOQUIC),
...
class Datapath(ofproto_protocol.ProtocolDesc):
    ...
    def _close_write(self):
        ...
        LOG.info("close datapath _write{0}".format(self.id))
        try:
    ...
    def close(self):
        LOG.info("close datapath {0}".format(self.id))
    ...
    def _recv_loop(self):
        ...
        while self.state != DEAD_DISPATCHER:
            try:
                ...
                LOG.info("EOFError, IOError close")
                break
