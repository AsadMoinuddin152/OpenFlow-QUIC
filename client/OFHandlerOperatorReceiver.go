...
var OFHandlerMapperRecv = make(map[byte]func([]byte, *OFHandler, UnixServerI) (int, error))
...
func MultipartReplyOperatorRecv(message []byte, ofh *OFHandler, Uds UnixServerI) (int, error) {
	leng, err := Uds.Write(message)
	return leng, err
}

func MultipartRequestOperatorRecv(message []byte, ofh *OFHandler, Uds UnixServerI) (int, error) {
...
func FlowModOperatorRecv(message []byte, ofh *OFHandler, Uds UnixServerI) (int, error) {
	leng, err := Uds.Write(message)
	return leng, err
}

func PacketOutOperatorRecv(message []byte, ofh *OFHandler, Uds UnixServerI) (int, error) {
	leng, err := Uds.Write(message)
	return leng, err
}

func PacketInOperatorRecv(message []byte, ofh *OFHandler, Uds UnixServerI) (int, error) {
	leng, err := Uds.Write(message)
	return leng, err
}

func DefaultOperatorRecv(message []byte, ofh *OFHandler, Uds UnixServerI) (int, error) {
	leng, err := Uds.Write(message)
	return leng, err
}
