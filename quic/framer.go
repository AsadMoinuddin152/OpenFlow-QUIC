...
type framer interface {
	...
	ReturnActiveStream(QueuingStreams []protocol.StreamID)
}
...
// ReturnActiveStream deep copy the whole slice and get the queuing order
func (f *framerI) ReturnActiveStream(QueuingStreams []protocol.StreamID) {
	f.mutex.Lock()
	copy(QueuingStreams,f.streamQueue)
	QueuingStreams[5] = protocol.StreamID(len(f.streamQueue))
	f.mutex.Unlock()
}