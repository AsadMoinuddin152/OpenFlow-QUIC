...
type Stream interface {
	...
	ReturnStreamRttStats() *utils.RTTStats
}
...
type SendStream interface {
	...
	/* EPM added */
	// GetStreamRttStats return RTT, the same as the connection one
	GetStreamRttStats() *utils.RTTStats
	// QueuingLen return buffer data length without mutex
	QueuingLen() int
	// MaxFrameLen return maximum frame max size length
	MaxFrameLen() int
...
type Session interface {
	...
	/*	EPM added	*/
	// ReturnSessionRttStats return connection RTT
	ReturnSessionRttStats() *utils.RTTStats
	// GetAllStreamQueue get all stream queuing length by framer
	GetAllStreamQueue(QueuingStream []protocol.StreamID)
