...
type sendStream struct {
	...
	/* EPM added */
	maxFrameLen protocol.ByteCount
...
// QueuingLen return the buffer length
// if buffer has no data, it tries to return the length of
// frame, which is ready to send.
func (s *sendStream) QueuingLen() int{
	if s.nextFrame == nil{
		return len(s.dataForWriting)
	}else {
		return int(s.nextFrame.DataLen())
	}
}
// MaxFrameLen return the maxFrameLen, normally, it would be
// larger than 1300 bytes
func (s *sendStream) MaxFrameLen() int {
	return int(s.maxFrameLen)
}
// GetStreamRttStats get RTT from flowController
func (s *sendStream) GetStreamRttStats() *utils.RTTStats {
	return s.flowController.GetRttStats()
}